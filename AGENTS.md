...
- 后端：Spring Boot 3.4.1 / Java 17 / Spring Security / JWT / MyBatis-Plus / Flyway / MySQL 8.4 / Redis。
...
## 2. 关键目录
- `backend/src/main/java/com/example/expense/`：auth、user、category、payment、platform、transaction、statistics、budget、recurring、export、imports、ocr、admin、businessaudit、common。
...
- `frontend/src/views/`：工作台、记一笔、流水、详情、分析、我的、分类、支付方式、线上平台、预算、周期记账、导入导出、后台管理、登录注册。
- `frontend/src/router/index.ts`、`frontend/src/stores/auth.ts`、`frontend/src/utils/preferences.ts`、`frontend/src/utils/quickAddDraft.ts`、`frontend/src/utils/calendarPicker.ts`、`frontend/src/utils/navigationBack.ts`、`frontend/src/utils/recordsRouteQuery.ts`：路由、登录态、本地偏好、记一笔草稿、日历选择、安全返回、流水路由查询。
- `frontend/tests/`：前端适配与回归脚本，例如工作台布局 fit、UI token 检查、通用日历、草稿、返回导航、底部导航焦点测试。
...
- 开发数据库与缓存：`docker compose -f docker-compose.dev.yml up -d`，MySQL 端口映射 `3307:3306`，Redis 端口映射 `6380:6379`。
...
- 前端：`cd frontend; npm install; npm run dev; npm run check:ui; npm run build`。
...
- 本地冒烟：后端、开发 MySQL 和 Redis 启动后运行 `.\scripts\smoke-local.ps1`；脚本默认清理本次创建的 `smoke_*` 测试数据，清理失败会返回非 0。
...
## 4. 业务与数据规则
- 认证使用 JWT access token + 数据库 refresh token；注册、登录和老用户补绑定邮箱都走邮箱验证码挑战；除注册邮箱验证码、注册、登录开始、登录验证码、绑定邮箱验证码、刷新 token、退出登录、Swagger 外，其余接口都要认证。
- Redis 用于邮箱验证码 challenge、登录失败限流、统计/推荐/基础数据缓存；认证短期状态依赖 Redis，不要把验证码、限流状态或缓存回落到本地内存。
- 当前用户 ID 只从 `SecurityUtils.currentUserId()` 获取；所有业务查询必须按 `userId` 过滤。
- 引用型数据（分类、支付方式、线上平台等）修改前先做归属校验，优先用对应 Service 的 `requireOwned`。
...
- 周期记账规则按用户隔离，支持 `MONTHLY` / `WEEKLY`、提前提醒、生成正式交易和跳过实例；工作台“今日待处理”只预览 2 条，其余通过紧凑入口进入完整列表。
- 后台管理只允许 `ADMIN_USERNAMES` 指定的已存在且未禁用用户访问；管理操作写入 `admin_audit_logs`，普通用户关键写操作写入 `business_audit_logs`，审计日志不得记录请求体、金额、备注、OCR 文本、密码或 token。
- 数据表主要是 `users`、`auth_challenges`、`refresh_tokens`、`categories`、`payment_methods`、`online_platforms`、`transactions`、`transaction_images`、`budgets`、`recurring_rules`、`recurring_rule_runs`、`import_jobs`、`admin_audit_logs`、`business_audit_logs`；改结构时同步 `schema.sql` 和 Flyway 迁移，已有 MySQL 数据卷不会自动重放初始化脚本。
...
- “记一笔”只保留默认三步流程：核心信息、分类场景、确认；不要重新添加“极简/进阶”模式切换或用户可见的模式说明。
- “记一笔”草稿按登录用户隔离保存在 `localStorage` 的 `expense.quickAddDraft.{userId}` 中，只保存表单字段、当前步骤、脏字段和 OCR 文本结果，不保存凭证图片文件本体。进入 `/quick-add` 时只提示用户选择“继续填写/丢弃”，不要自动套用草稿；从底部悬浮按钮带 `type=EXPENSE` / `type=INCOME` 进入时，只提示同类型草稿。保存记录成功后必须清除当前用户草稿。
- 交易表单和编辑记录表单保持高频顺序：类型、金额、事项、分类、支付方式；其他信息放补充区，保存用底部固定操作栏。金额输入展示应带人民币符号 `¥`，但表单值和接口 payload 仍只保留数字金额。
- “记一笔”确认页摘要中的时间应使用面向用户的格式（如 `2026年06月05日 10:29`），不要直接展示 `datetime-local` 的内部 `T` 分隔格式。
- 流水页记录左滑优先交给 `van-swipe-cell`，日期横滑只作用于非记录行区域。
- 流水页支持日卡片和时间线两种模式；时间线模式记录行时间只显示 `HH:mm`，滚动后动态显示返回顶部按钮，顶部时隐藏。
- 流水页“跳转日期”和全站日期/月份/年份/日期时间选择入口统一使用 `frontend/src/components/ModernDateField.vue`；date/datetime 月历从周一开始，流水跳转只能选择当前筛选结果中有记录的日期。
- 流水页当天初始展示条数放在“我的”页偏好设置中，默认 5 条。
- 选择器、筛选和管理表单类底部弹窗统一使用 `frontend/src/components/BottomSheet.vue`；普通页面不要直接写 `<van-popup>`。仅 `BottomSheet` 自身、地图选点弹窗和管理端详情抽屉允许作为特殊例外直接使用 `van-popup`，且仍必须 `teleport="body"`。
- 记一笔分类、支付方式、线上平台选择弹窗的 `.quick-choice-list` 是唯一纵向滚动容器，必须保留 `@touchmove.stop`，避免 Vant Popup 锁滚动拦截移动端滑动。
- 页面返回入口统一使用 `frontend/src/utils/navigationBack.ts` 的安全返回逻辑，避免新标签页直达后 `router.back()` 退到浏览器空白页。
...
- 新增或调整认证验证码、登录限流、缓存或 Redis 依赖时，同步检查 `.env.example`、`.env.prod.example`、`docker-compose.*.yml`、`docs/api.md`、`docs/production-runbook.md` 和相关测试。
- 新增或调整周期记账、后台管理、审计日志能力时，同步检查 Flyway 迁移、`docker/mysql/init/01_schema.sql`、`docs/api.md`、前端类型/API 服务和相关页面。
- 后端改动至少跑 `cd backend; mvn test`。
- 前端改动至少跑 `cd frontend; npm run check:ui; npm run build`；涉及工作台首页布局、预览数量、洞察卡片或移动端首屏适配时，额外跑 `frontend/tests/workspace-fit.mjs`。
