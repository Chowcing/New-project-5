...
验证码挑战保存于 Redis，最多允许 5 次错误尝试，验证成功后一次性消费；同一邮箱验证码 latest 指针用于注册校验和 60 秒重发限制。登录失败限流也保存于 Redis，key 使用用户名和客户端 IP 的 SHA-256，不暴露原文。生产环境应配置 SMTP；本地开发未配置 SMTP 时可通过后端日志查看验证码，生产不应开启本地日志模式。
