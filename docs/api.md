...
- `GET /transactions/recommendations/quick-entry?type=EXPENSE&limit=10`：极简记账推荐，返回按置顶、最近使用、使用频率、排序综合排列的分类、支付方式、线上平台、线下地点和推荐组合；分类等候选最多按 `limit` 返回，推荐组合最多返回 6 个
...
前端“记一笔”草稿是本地交互能力，不新增后端接口。草稿按登录用户隔离存放在 `localStorage` 的 `expense.quickAddDraft.{userId}` 中，内容包括表单字段、极简/进阶模式、进阶步骤、用户手动修改标记和 OCR 文本结果；缺少当前用户 ID 时不读写草稿。进入 `/quick-add` 时如果存在有效草稿，页面只提示用户选择“继续填写”或“丢弃”，不会自动套用；从底部悬浮按钮带 `type=EXPENSE` 或 `type=INCOME` 进入时，只提示同类型草稿。保存记录成功后前端清除当前用户草稿。
