...
请求图片复用交易凭证图片规则：单张最大 3MB，仅支持 JPG、PNG、WebP，并校验文件内容与声明类型一致。首版默认关闭，不自动创建交易，不落库识别文本，也不记录请求体或识别文本日志。默认配置 `OCR_ENABLED=false`、`OCR_PROVIDER=disabled` 时，合法图片请求返回 `400` 和“图片转文字功能未启用”。
