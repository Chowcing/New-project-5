...
交易图片规则：图片非必传，单笔最多 3 张，单张最大 3MB，仅支持 JPG、PNG、WebP。记录响应中的 `images` 包含 `id`、`originalFilename`、`contentType`、`sizeBytes`、`url`、`sortOrder`。图片 URL 仍需携带 `Authorization` 请求，不作为公开静态资源访问。删除图片或流水后，物理文件默认保留 7 天，再由后台任务逐个明确路径清理；保留期可通过 `TRANSACTION_IMAGE_RETENTION_DAYS` 调整。
...
请求图片复用交易凭证图片规则：单张最大 3MB，仅支持 JPG、PNG、WebP，并校验文件内容与声明类型一致。OCR 不自动创建交易，不落库识别文本，也不记录请求体或识别文本日志。默认配置 `OCR_ENABLED=false`、`OCR_PROVIDER=disabled` 时，合法图片请求返回 `400` 和“图片转文字功能未启用”。
...
- `400 仅支持 JPG、PNG、WebP 图片` / `图片文件内容与类型不匹配` / `单张图片不能超过 3MB`：上传图片不符合交易凭证图片规则
